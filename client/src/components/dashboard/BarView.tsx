...
const LiveTimer: React.FC<{ createdAt: string; estimatedMins?: number }> = ({ createdAt, estimatedMins = 10 }) => {
...
export const BarView: React.FC = () => {
  ...
  // 1. Compile all bar items
  const allBarItemsMap = new Map<string, any>();
  ...
  queueItems.forEach(qi => {
    ...
    if (prepArea === 'BAR' || qi.assigned_staff_id === 106) {
      const order = orders.find(o => o.id === qi.order_id);
      allBarItemsMap.set(`queue-item-${qi.id}`, {
        ...qi,
  ...
  orders.forEach(order => {
    (order.items || []).forEach(item => {
      ...
      if (prepArea === 'BAR' || prepArea === 'BEVERAGE' || prepArea === 'DRINK') {
        ...
        if (!hasQueueItem) {
          allBarItemsMap.set(`order-item-${item.id}`, {
            id: item.id,
            order_number: order.order_number,
            menu_item_name: item.menu_item_name,
            quantity: item.quantity,
            preparation_area: 'Bar' as const,
  ...
  const rawItems = Array.from(allBarItemsMap.values());

  // Quick Metrics
  const pendingCount = rawItems.filter(i => i.status === 'Pending').length;
  const preparingCount = rawItems.filter(i => i.status === 'Preparing').length;
  ...
  // Get active bar categories for station filtering (dynamic)
  const barCategories = categories.filter(c => 
    menuItems.some(m => m.category_id === c.id && m.preparation_area === 'Bar')
  );

  const columns = [
    { id: 'Pending', label: 'Pending', status: 'Pending', color: 'bg-slate-50', headerColor: 'text-slate-600', borderColor: 'border-slate-200' },
    { id: 'Preparing', label: 'Preparing', status: 'Preparing', color: 'bg-blue-50/30', headerColor: 'text-blue-600', borderColor: 'border-blue-200' },
      ...
      <div className="flex-none rounded-xl bg-white border border-slate-200 p-4 flex flex-col md:flex-row justify-between gap-4 shadow-sm">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-lg font-black text-slate-900 leading-tight">BDS Dashboard</h2>
              <p className="text-xs text-slate-500 font-medium">Real-time Bar Display System</p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-black text-blue-600 leading-none">{preparingCount}</div>
              <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Preparing</div>
            </div>
            <div className="text-center">
                  ...
                  {colItems.map((item) => (
                    <motion.div
                      ...
                      className={`relative bg-white rounded-xl p-4 shadow-sm border ${
                        item.status === 'Preparing' ? 'border-blue-200 shadow-blue-100' :
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <LiveTimer createdAt={item.created_at} estimatedMins={10} />
                      </div>
                      ...
                      <div className="py-3 border-t border-b border-slate-100">
                        <div className="font-display text-base font-black text-slate-900">
                          <span className="text-blue-500 mr-1">{item.quantity}x</span>
                          {item.menu_item_name}
                        </div>
                      ...
                      <div className="mt-4 flex gap-2">
                        {item.status === 'Pending' && (
                          ...
                          >
                            <Play className="h-4 w-4 fill-white" /> Mix Drink
                          </button>
                        )}
