...
class BarService {
  async getQueue() {
    const queue = await prisma.barQueue.findMany({
      where: {
        status: {
          notIn: [BarStatus.SERVED, BarStatus.CANCELLED]
        }
      },
      include: {
        orderItem: {
    ...
    return queue.map(q => ({
      ...
      assigned_staff_id: q.assignedBartenderId
    }));
  }

  async updateStatus(id: number, status: BarStatus) {
    const queueItem = await prisma.barQueue.update({
      where: { id },
      data: { status },
    ...
    const mapped = {
      ...
      assigned_staff_id: queueItem.assignedBartenderId
    };

    socketService.emitToRoom(SOCKET_ROOMS.BAR, SOCKET_EVENTS.BAR_QUEUE_UPDATED, mapped);
  ...
  async assignStaff(id: number, staffId: number) {
    const queueItem = await prisma.barQueue.update({
      where: { id },
      data: { assignedBartenderId: staffId },
      include: { orderItem: { include: { order: true } } }
    });
